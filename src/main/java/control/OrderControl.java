		...
		try {
			...
			String deliveryAddress = request.getParameter("deliveryAddress");
			 ...
			 HttpSession session = request.getSession();
				...
				email.setFrom("baobao161102@gmail.com"); //chinh lai email quan tri tai day [chu y dung email con hoat dong]
				email.setFromPassword("atqg itvn xith xikd"); //mat khau email tren
				email.setTo(emailAddress);
				email.setSubject("Dat hang thanh cong tu Bao Giay CHB");
				StringBuilder sb = new StringBuilder();
				sb.append("Dear ").append(name).append("<br>");
				sb.append("Ban vua dat dang tu Bao Giay CHB. <br> ");
				...
				for(Cart c : list) {
					for(Product p : list2) {
						if(c.getProductID()==p.getId()) {
							sb.append(p.getName()).append(" | ").append("Price:").append(p.getPrice()).append("đ").append(" | ").append("Amount:").append(c.getAmount()).append(" | ").append("Size:").append(c.getSize()).append("<br>");
						}
					}
				}
				sb.append("Tong Tien: ").append(String.format("%.02f",totalMoneyVAT)).append("đ").append("<br>");
				sb.append("Cam on ban da dat hang tai  Bao Giay CHB<br>");
